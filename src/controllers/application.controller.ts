...
export class ApplicationController{
    ...
    async getApplicationById(req: Request, res: Response): Promise<void> {
        ...
        if (typeof id !== "string" || !id.trim()) {
            res.status(400).json({ error: "Application ID format incorrect. Need String." });
            return;
        }
		try {
    ...
    async deleteApplication(req: Request, res: Response): Promise<void> {
        ...
        try {
            const deletedApplication = await this.applicationService.deleteApplication(id);
            if (deletedApplication) {
                res.status(200).json({ message: "Application deleted successfully.", application: deletedApplication });
            } else {
                res.status(404).json({ error: "Application no longer exists."});
            }
        } catch (error) {
    ...
    async getApplicationsForUser(req: Request, res: Response): Promise<void> {
        ...
        try {
            const applications = await this.applicationService.getApplicationsForUser(userId);
            res.status(200).json(applications);
        } catch (error) {
            console.error(`Error fetching applications for user with id ${userId}:`, error);
            res.status(500).send();
        }
    }
