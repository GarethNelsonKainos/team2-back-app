...
async function main() {
    ...
    const softwareApprentice = await prisma.jobRole.create({
        data: {
            ...
            location: 'Belfast',
            description: 'As a Software Engineer at Kainos, you will be responsible for designing, developing, and maintaining software applications that meet our clients\' needs. You will work closely with cross-functional teams to deliver high-quality solutions and contribute to the overall success of our projects.',
            ...
            sharepointUrl: 'https://kainossoftwareltd.sharepoint.com/sites/Career/JobProfiles/Engineering/Job%20profile%20-%20Apprentice%20Software%20Engineer%20(Apprentice).pdf',
            closingDate: new Date('2026-03-01T00:00:00Z'),
            capabilityId: engineering_capability.capabilityId,
            bandId: apprentice.bandId,
            statusId: ((await prisma.status.findFirst())!.statusId || 'No status recorded'),
        },
    });
    ...
    const testEngAssociate = await prisma.jobRole.create({
        data: {
            ...
            sharepointUrl: 'https://kainossoftwareltd.sharepoint.com/sites/Career/JobProfiles/Engineering/Job%20profile%20%20Test%20Engineer%20(Associate).pdf',
            closingDate: new Date('2026-03-01T00:00:00Z'),
            capabilityId: testing_and_quality_assurance.capabilityId,
            bandId: associate.bandId,
            statusId: ((await prisma.status.findUniqueOrThrow({where: {statusName: 'Closed'}}))!.statusId || 'No status recorded'),
        },
    });
    ...
    const innovationLeadConsultant = await prisma.jobRole.create({
        data: {
            ...
            sharepointUrl: 'https://kainossoftwareltd.sharepoint.com/sites/Career/JobProfiles/Engineering/Job%20profile%20-%20Innovation%20Lead%20(Consultant).pdf',
            closingDate: new Date('2026-03-01T00:00:00Z'),
            capabilityId: engineering_strategy_and_planning.capabilityId,
            bandId: apprentice.bandId,
            statusId: ((await prisma.status.findUniqueOrThrow({where: {statusName: 'In Progress'}}))!.statusId || 'No status recorded'),
        },
    });
    console.log('Created JobRole:', softwareApprentice);
    ...
    const technicalArchitect = await prisma.jobRole.create({
        data: {
            ...
            numberOfOpenPositions: 1,
            sharepointUrl: 'https://kainossoftwareltd.sharepoint.com/sites/Career/JobProfiles/Engineering/Job%20Profile%20-%20Technical%20Architect%20(Consultant).pdf',
            closingDate: new Date('2026-03-01T00:00:00Z'),
